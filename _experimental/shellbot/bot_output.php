<?php
	...
	if (@$_SESSION[ADMIN_SESSION_PRE_."chatsgrpandtsonadmincheck"] == @$_GET["chatsgrpandtsonadmincheck"] AND @$_GET["chatsgrpandtsonadmincheck"] != NULL) {
		...
		if (@$_SESSION[ADMIN_SESSION_PRE_."chatsgrppostsonadmincheck"] == @$_POST["chatsgrppostsonadmincheck"] AND @$_POST["chatsgrppostsonadmincheck"] != NULL AND @$_POST["checking"] == "ok") {	
						...
						$flag = false;
				$query12 = "SELECT * FROM `"._BTM_TABLE_CONSGROUPTE_."` INNER JOIN "._BTM_TABLE_CONSGROUPSDB_." ON "._BTM_TABLE_CONSGROUPSDB_.".id = "._BTM_TABLE_CONSGROUPTE_.".group WHERE "._BTM_TABLE_CONSGROUPTE_.".text = \"".str_replace('"', '\"',$xtitle)."\" AND "._BTM_TABLE_CONSGROUPSDB_.".id = \"".htmlspecialchars($_POST["catid"])."\"";
				$result12 = mysqli_query($mysql, $query12) or die(mysqli_error($mysql));
				while ($row = @mysqli_fetch_array($result12, MYSQLI_BOTH)) {	
					$flag = true;
				}					
...
<div id="sectionheading" >Areas</div>
<center><font style="color: yellow; font-size: 12px;">Here are Groups!<br />
		...
		<br /><br />
				...
				<a href="./?location=bugfish&2ndloc=chat&coloc=wgroups&group=botoutput"  style="color: lightgrey !important;">Bot Outputs</a> <!--| 
...
<?php
if (@$_GET["show"] != NULL AND @$_GET["show"] != "" AND @$_GET["type"] == "botoutput") {
?>
<section id="newsection">
<div id="sectionheading" >Change Text</div>
<center><font style="font-size: 12px;">CHANGE TEXT FOR SELECTED AREA</font></center>
<tr style="width: 100%;font-size: 14px;"><?php echo '<form method="post" action="./?location=bugfish&2ndloc=chat&group=botoutput&coloc=wgroups&on=announce&show='.@htmlspecialchars($_GET["show"]).'">'; ?>
	...
	<?php
		echo '<table style="width: 100%;" cellpadding="10" cellspacing="0">';
			...
			while ($row = mysqli_fetch_array($result, MYSQLI_BOTH)) {
					...
					echo "<td style='width: 25%;'><a id='thedelbutton' href='./?location=bugfish&2ndloc=chat&coloc=wgroups&group=botoutput&show=".@htmlspecialchars($_GET["show"])."&on=text&op=delete&id=".$row["id"]."&chatsgrpandtsonadmincheck=".$_SESSION[ADMIN_SESSION_PRE_."chatsgrpandtsonadmincheck"]."'>Delete</a>";
					echo '</tr>';
				$flag = TRUE;
			}
		echo '</table><br /><br />';
...
<tr style="width: 100%;font-size: 14px;"><form method="post" action="./?location=bugfish&2ndloc=chat&group=botoutput&coloc=wgroups&on=announce">
	<td style='width: 25%;'><input type='text' name='xtitle' placeholder="NAME"></td>
	<input type="hidden" name='xtext' value="botoutput">
	<?php echo '<input type="hidden" name="chatsgrppostsonadmincheck" value="'.$_SESSION[ADMIN_SESSION_PRE_."chatsgrppostsonadmincheck"].'">';  ?>
	<td style='width: 25%;'><input type='submit' value='+'></td>
</form></tr><br clear="left">
</section>
	...
	echo '<div id="sectionheading" >[Global]</div>';
		...
		echo '<table style="width: 100%;" cellpadding="10" cellspacing="0">';
			...
			while ($row = mysqli_fetch_array($result, MYSQLI_BOTH)) {
					...
					echo '<tr style="width: 100%;font-size: 14px;">';
					echo "<td style='width: 75%;'><a href='./?location=bugfish&2ndloc=chat&coloc=wgroups&type=botoutput&group=botoutput&show=".$row["id"]." '>".$row["title"]." [ ".$counter." ]</a></td>";
					echo "<td style='width: 25%;'><a id='thedelbutton' href='./?location=bugfish&2ndloc=chat&coloc=wgroups&group=botoutput&on=announce&op=delete&id=".$row["id"]."&chatsgrpandtsonadmincheck=".$_SESSION[ADMIN_SESSION_PRE_."chatsgrpandtsonadmincheck"]."'>Delete</a>";
					echo '</tr>';
				$flag = TRUE;
			}
		echo '</table>';
